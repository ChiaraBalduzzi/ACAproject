...
void sequencing(char seq[], char pat[], long long int s_len, long long int p_len, conteggio *c) {
	...
	#pragma omp single
	{
	#pragma omp task firstprivate (err_cnt,count_L_hole) private (i,j)
	{
	#pragma omp parallel for private(err_cnt,count_L_hole,j) schedule(dynamic,p_len)
	...
	} // pragma omp task (forward) ends here
	...
	} // pragma omp task (backward) ends here

	} // pragma omp singel ends here
